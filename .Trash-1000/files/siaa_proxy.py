...
CHUNK_SIZE             = 800
CHUNK_OVERLAP          = 200
MAX_CHUNKS_CONTEXTO    = 4   # 4 × 800 = 3200 chars máx por doc
...
SYSTEM_DOCUMENTAL = """Eres SIAA (Sistema Inteligente de Apoyo Administrativo), asistente oficial de la Seccional Bucaramanga de la Rama Judicial de Colombia.

REGLAS SIN EXCEPCIÓN:

1. Responde ÚNICAMENTE con información del contexto [DOC:...] proporcionado.
2. PROHIBIDO usar conocimiento propio, externo o de entrenamiento.
...
4. Responde en español formal. Máximo 8 líneas.
5. Si hay artículos o pasos numerados en el contexto, cítalos en orden.
6. NUNCA inventes, supongas ni completes con conocimiento propio.
7. Cuando el contexto tiene separadores ═══, cada bloque es un documento diferente. No mezcles información entre bloques.

CITACIÓN OBLIGATORIA — última línea de cada respuesta:
📄 Fuente: [NOMBRE DOC] › Sección: [TÍTULO] › Párrafo [N]

NUNCA termines sin la línea 📄 Fuente."""
...
def puntuar_chunk(chunk: dict, palabras: set, pregunta_norm: str,
                  terminos_prio: set) -> float:
    """
    Puntúa un chunk según su relevancia para la pregunta.

    Sistema de puntuación:
      +3  por palabra prioritaria encontrada
      +1  por palabra normal encontrada
    ...
    for w in palabras:
        if w in texto:
            puntos += 3.0 if w in terminos_prio else 1.0

    # Pregunta completa
    if pregunta_norm in texto:
        puntos += 15.0

    # [FIX-2] Bonus artículo con grado (máxima prioridad)
...
def extraer_fragmento(nombre_doc: str, pregunta: str) -> str:
    ...
    scored = []
    for chunk in chunks:
        pts = puntuar_chunk(chunk, palabras, pregunta_norm, terminos_prio)
        if pts > 0:
